...
public final class Value implements Undef, Null, Bool, Num, Str, DeepImmutable {
    ...
    //erikwazhere, added these for debugging ease
    boolean isMaybeAbsent, isUnknown, isMaybePresent, isNone, isPolymorphic, isMaybeUndef, isMaybeNull, isNotPresent, isNotAbsent;
    /**
    ...
    public Map<SourceLocation, Value> valueHistory = new LinkedHashMap<>();
    /**
     * Hash code.
     */
    ...
    private Value bindOverride = null;
    private String typePrefixTrack = "";
    static {
        init();
    }
    ...
    private Value() {
        ...
        var = null;
        valueHistory = new LinkedHashMap<>();
        hashcode = 0;
        update(); //erikwazhere
    }
    ...
    private Value(Value v) {
        ...
        var = v.var;
        hashcode = v.hashcode;
        valueHistory = new LinkedHashMap<>(v.valueHistory);
        update(); //erikwazhere
    }
    ...
    private static Value canonicalize(Value v) {
        ...
        int slHash = 0;
        if (v.valueHistory != null){
            for (SourceLocation sl: v.valueHistory.keySet()){
                slHash += sl.hashCode();
            }
        }
    ...
    public void update(){
        this.isMaybeAbsent = isMaybeAbsent();
        this.isUnknown = isUnknown();
        this.isMaybePresent = isMaybePresent();
        this.isNone = isNone();
        this.isPolymorphic = isPolymorphic();
        this.isMaybeUndef = isMaybeUndef();
        this.isMaybeNull = isMaybeNull();
        this.isNotPresent = isNotPresent();
        this.isNotAbsent = isNotAbsent();
    ...
    public boolean isMaybeAbsent() {
        if (object_labels != null && !object_labels.isEmpty()){
            for (ObjectLabel ol : object_labels){
                if (ol.getKind() == Kind.FUNCTION && ol.getSourceLocation().toString().contains("call-all-functions")){
                    return false;
                }
            }
        }
    ...
    public Value removeAttributes() {
        ...
        //r.valueHistory = this.valueHistory;
    ...
    public Value setAttributes(Value from) {
        ...
        //r.valueHistory = from.valueHistory;
    ...
    public Value join(Value v) {
        if (v == this)
            return this;
        Value r = new Value(this);
        //r.valueHistory = v.valueHistory;

        if (r.joinMutable(v)) {
            Value conver= canonicalize(r);
            if (r.isMaybeAbsent() && !this.isMaybeAbsent() && conver.isMaybeAbsent() && !this.valueHistory.isEmpty()){
                r = new Value(this);
                if (v.joinMutable(r)){
                    conver= canonicalize(v);
                } else {
                    return this;
                }
    ...
    private boolean joinMutable(Value v) {
        ...
        if (!v.isPolymorphic()) {
            ...
            for (Map.Entry<SourceLocation, Value> ent : v.valueHistory.entrySet()){
                this.valueHistory.put(ent.getKey(), ent.getValue());
            }
            //this.valueHistory = v.valueHistory;
            // objects
            if (v.object_labels != null) {
...
//            any = true;
    ...
    public boolean strEquals(String cmp){
        if (str == null || isPolymorphicOrUnknown()){
            return false;
        }
        return str.equals(cmp);
    }
    public String getStrSafely(){
        return str;
    }

    private void reconstituteHistory(Map<SourceLocation, Value> vh, ReconstitutedString rs){
        for (Map.Entry<SourceLocation, Value> ent : vh.entrySet()) {
            // TODO: this will have to deal with multiple values at some point -Erik
            Value currentVal = ent.getValue();
            rs.append(currentVal.getStr(), ent.getKey(), ent.getValue());
            if (!currentVal.valueHistory.isEmpty()){
                Value vhFirst = currentVal.valueHistory.entrySet().iterator().next().getValue();
                if (vhFirst != currentVal) {
                    reconstituteHistory(currentVal.valueHistory, rs);
                }
            }
        }
    }

    public ReconstitutedString reconstituteString(){
        ReconstitutedString rs = new ReconstitutedString();
        //System.out.println(this.valueHistory);
        reconstituteHistory(this.valueHistory, rs);
        return rs;
    }
    /**
     * Looks through all the value histories with static SourceLocations and returns the ones that contain the
     * desired search value.
     * @param searchFor
     * @return a set of  <SourceLocation, Value>
     */
    public Set<Map.Entry<SourceLocation, Value>> findSourceLocationsContaining(Set<String> searchFor, String prefix) {
        Map<SourceLocation, Value> foundSLs = new LinkedHashMap<>();
        for (Map.Entry <SourceLocation, Value> ent : valueHistory.entrySet()){
            SourceLocation sl = ent.getKey();
            if (!sl.isDynamic()){
                Value sourceVal = ent.getValue();
                String temp = prefix + sourceVal.getStrSafely();
                if (sourceVal.getStrSafely() != null && searchFor.contains(temp)){
                    sourceVal.setTypePrefixTrack(prefix);
                    foundSLs.put(sl,sourceVal);
                }
            }
        }
        return foundSLs.entrySet();
    }
     ...
     * Constructs the value describing the given string.
     */
    public static Value makeStr(String s, SourceLocation sourceLocation) {
        Value r = new Value();
        r.str = s;
        r.valueHistory.put(sourceLocation, r);
        return canonicalize(r);
    }
    ...
    public Value restrictToStr() {
        ...
        r.update(); //erikwazhere
        r.str = str;
        // r is not built with Value(this), adding history
        r.valueHistory = new LinkedHashMap<>(valueHistory);
        return canonicalize(r);
    }
...
//    }
    ...
    public Map<SourceLocation, Value> getSourceLocations(){

        return valueHistory;
    }

    /*public void addSourceLocation(String strVal, SourceLocation sl){
        this.valueHistory.put(strVal, sl);
    }*/
    public void addSourceLocations(Value newVals){
        newVals.valueHistory.forEach(valueHistory::putIfAbsent);
    }

    public boolean addBindOverride(Value bindOverride){
        boolean setOverride = false;
        for (ObjectLabel ol : this.getObjectLabels()){
            if (ol.getKind() == Kind.FUNCTION){
                if (ol.hasFunction()) {
                    ol.getFunction().addBindOverride(bindOverride);
                    setOverride=true;
                }
            }
        }
    ...
    public void setTypePrefixTrack(String type){
        typePrefixTrack=type;
    }
    public String getTypePrefixTrack(){
        return typePrefixTrack;
    }
}
