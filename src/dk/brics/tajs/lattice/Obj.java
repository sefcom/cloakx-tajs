...
//    }
    ...
    public void setDefaultArrayProperty(Value v) {
        ...
        if (!v.isUnknown() && v.isMaybePresent() && !v.isMaybeAbsent()) {
            String erikwazCHANGEDhere="Commented out this exception, not sure what it means or how to fix.";
            // made for completion of hia2 (jpq and options.js)
            //throw new AnalysisException("Illegal default array property: " + v + " isKnown and isMaybePresent and isNotMaybeAbsent");
            default_array_property = v.restrictToUndef();
        } else {
...
//            throw new AnalysisException("Illegal default nonarray property: " + v);
    ...
    public String toString() {
        ...
        if (default_array_property.isNone()) {
            any = true;
            b.append("<none>");
        } else if (default_array_property.isUnknown()) {
            any = true;
            b.append("<unkown>");
        } else if (default_array_property.isNotPresent()) {
            any = true;
            b.append("<absent>");
        }
        for (Entry<String, Value> me : sortedEntries(properties)) {
