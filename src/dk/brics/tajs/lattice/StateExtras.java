...
public class StateExtras {
    ...
    private Map<String, Map<String, Value>> may_maps_values;
    ...
    protected StateExtras(StateExtras x) {
        if (Options.get().isCopyOnWriteDisabled()) {
            ...
            may_maps_values = new HashMap<>(x.may_maps_values);
        } else {
            may_sets = x.may_sets;
            writable_may_sets = x.writable_may_sets = false;
            may_maps = x.may_maps;
            may_maps_default = x.may_maps_default;
            may_maps_values = x.may_maps_values;
    ...
    private void makeMayMapsWritable() {
        ...
        may_maps_values = new HashMap<>(may_maps_values);
    ...
    public boolean isNone() {
        ...
        for (Map<String, Set<ObjectLabel>> s1 : may_maps.values())
            for (Set<ObjectLabel> s2 : s1.values())
                if (!s2.isEmpty())
                    return false;
        for (Map<String,Value> vm : may_maps_values.values()){
            if (!vm.isEmpty()) {
                return false;
            }
    ...
    public void addValueToMayMap(String name, String key, Value value){
        makeMayMapsWritable();
        if (key != null && value != null) {
            may_maps_values.computeIfAbsent(name, k -> newMap());
            may_maps_values.get(name).put(key, value);
            String slstr = "";
            if (value.getSourceLocations() != null) {
                for (SourceLocation sl : value.getSourceLocations().keySet()) {
                    if (slstr.isEmpty()) {
                        String[] strarr = sl.getStaticLocation().getFile().split("/");
                        slstr = strarr[(strarr.length-1)] + " ";
                    }
                    slstr += sl.getLineNumber() + ":" + sl.getColumnNumber() + "->" + sl.getEndLineNumber() + ":" + sl.getEndColumnNumber();
                }
                //addFiles.warn("\tadded to ValMayMap: " + key + ": " + value + " [" + slstr + "]");
            }
        }
    }
    ...
    /**
     * Retrieves a value for given name and key .
     */
    public Value getFromMayMapValues(String name, String key) {

        if (may_maps_values.get(name) == null){
            return null;
        }
        return may_maps_values.get(name).get(key);
    }

    public Value getFromMayMapValues(String name) {
        Value v = null;
        if (may_maps_values.containsKey(name)) {
            for (Map.Entry<String, Value> met : may_maps_values.get(name).entrySet()) {
                if (v == null) {
                    v = met.getValue();
                } else {
                    v.join(met.getValue());
