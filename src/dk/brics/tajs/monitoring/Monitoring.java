...
//		}
    ...
    public void visitReadProperty(ReadPropertyNode n, Set<ObjectLabel> objlabels, Str propertystr, boolean maybe, State state, Value v, ObjectLabel global_obj) {
        ...
        if (n.isPropertyFixed()) {
            String extras = "";
            if (n.getBaseRegister() < state.getRegisters().size() && n.getBaseRegister() != -1) {
                Value base = state.getRegisters().get(n.getBaseRegister());
                if (base != null){
                    extras = extras + " Base=" + base;
                }
            }
            if (n.getPropertyRegister() < state.getRegisters().size()  && n.getPropertyRegister() != -1) {
                Value preg = state.getRegisters().get(n.getPropertyRegister());
                if (preg != null){
                    extras = extras + " Base=" + preg;
                }
            }
            if (objlabels.stream().anyMatch(l -> l.isHostObject() && !l.equals(global_obj))) {
                ...
                if (isPartial) {
                    addMessage(n, s, Severity.MEDIUM, "***** Reading absent property (1)" + n.getPropertyString() + " of partially modeled host object!" + " " + extras);
                } else {
                    addMessage(n, s, Severity.MEDIUM, "***** ERROR Reading absent property (2)" + n.getPropertyString() + " of host object \n\n\tThis probably halted EXECUTION of JS!! \n\n" + " " + extras);
                }
            } else {
                addMessage(n, s, Severity.MEDIUM, "***** Reading absent property (3)" + n.getPropertyString() + " " + extras);
            }
        } else {
            addMessage(n, s, Severity.LOW, " Reading absent property (computed name) (4)");
        }
        Status s2;
        v = UnknownValueResolver.getRealValue(v, state);
