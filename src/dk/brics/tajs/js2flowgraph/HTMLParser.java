...
public class HTMLParser {
    ...
    private boolean typeMatchedClass = false;
    private boolean typeMatchedId = false;
    public boolean typeMatchedId(){
        return typeMatchedId;
    }
    public boolean hasMatchedClass(){
        return typeMatchedClass;
    }
    public Set<Attribute> findAttribute(Set<String> searchFor, Set<String> searchType) {
        Set<Attribute> matches = new HashSet<>();
        for (StartTag st : doc.getAllStartTags()) {
            for (Attribute a : st.getAttributes()) {
                if (a.getKey().toLowerCase().equals("class") && searchFor.contains("."+ a.getValue())) {
                    typeMatchedClass = true;
                    matches.add(a);
                }
                if (searchType.contains(a.getKey().toLowerCase()) && searchFor.contains("#"+a.getValue())) {
                    typeMatchedId=true;
                    matches.add(a);
                }
            }
