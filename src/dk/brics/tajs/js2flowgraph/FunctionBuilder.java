...
//        boolean expressionContext = kind == FunctionDeclarationTree.Kind.ARROW || kind ==  FunctionDeclarationTree.Kind.EXPRESSION ||
//                (env.getUnevalExpressionResult() != null && env.getUnevalExpressionResult().resultRegister == env.getResultRegister());
    ...
    public TranslationResult process(SwitchStatementTree tree, AstEnv env) {
        ...
        if (defaultCase == null) {
            ...
            ImmutableList<ParseTree> defaultsBody = null;
            if (!lastCaseIsDefaultCase) {
                ...
                // Despite the comment above, programmers do weird stuff in the wild, which includes having the default statement not at the end of a case statement
                // So, in the case of the default is "effectively last" but included with other case branches
                // I'm forcing it there, this shouldn't impact program execution -Erik
                String erikwazhere="";
                for (int i = 0; i < tree.caseClauses.size(); i++){
                    if (tree.caseClauses.get(i).type == ParseTreeType.DEFAULT_CLAUSE && defaultCase.statements.isEmpty()){
                        for (int j=i+1; j < tree.caseClauses.size(); j++){
                            if (!tree.caseClauses.get(j).asCaseClause().statements.isEmpty() && j == (tree.caseClauses.size() -1) ){
                                // all statements to the end are empty, so default is effectively at end
                                defaultsBody = ImmutableList.copyOf(tree.caseClauses.get(j).asCaseClause().statements) ;
                            }
                        }
                    }
                }
            ...
            // 5.2 handle body
            if (defaultsBody != null) {
                processedSequence = processList(defaultsBody, caseEnv.makeAppendBlock(defaultBlock));
            } else {
