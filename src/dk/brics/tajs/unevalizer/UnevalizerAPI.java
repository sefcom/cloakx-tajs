...
public class UnevalizerAPI {
    ...
    private static TreeMap<Integer, Integer> createUnevalSourceMap(String orig, String compd){
        int adjustment = 0;
        TreeMap<Integer, Integer> unevalMap = new TreeMap<>();
        for (int i = 0; (i+adjustment) < orig.length() && i < compd.length(); i++){

            if (orig.charAt(i+adjustment) == compd.charAt(i)) {

            } else {
                String encS= "\\x" + Integer.toHexString((int) orig.charAt(i+adjustment));
                if (!encS.isEmpty() && (i+encS.length()) < compd.length() && encS.equals(compd.substring(i,i+encS.length()))){
                    adjustment -= (encS.length()-1);
                    i += encS.length();
                    unevalMap.put(i, adjustment);
                    i += -1;
                } else {
                    adjustment++;
                    unevalMap.put(i, adjustment);
        ...
        // if strings are same add a single entry with a 0 adjustment for any value.
        if (unevalMap.isEmpty()){
            unevalMap.put(orig.length(), 0);
        }
        return unevalMap;
    ...
    public static Value evaluateEvalCall(CallInfo call, GenericSolver<State, Context, CallEdge, IAnalysisMonitoring, Analysis>.SolverInterface c, State state, CallNode callNode) {
        ...
        String callName = UnevalTools.get_call_name(currentFg, callNode);
        boolean aliased_call = !"eval".equals(callName); // TODO: aliased_call should also affect the execution context?
        aliased_call &= !"executeScript".equals(callName);  // doesn't seem to execute aliased call ,not sure why, but executeScript should be treated same as eval
        aliased_call &= !"setAttribute".equals(callName);
        ...
        if (e == null || !e.getKey().equals(unevaledSubst)) {
            e = FlowGraphMutator.extendFlowGraph(currentFg, unevaled, unevaledSubst, e, callNode, false, var);
            TreeMap<Integer, Integer> unevalSourceMap = createUnevalSourceMap(input.getNormalForm(), unevaled);
            for(BasicBlock bb : e.getBlocks()){
                for (AbstractNode n : bb.getNodes()){
                    n.getSourceLocation().setUnevalSourceMap(unevalSourceMap);
                }
            }
        }
