...
public class SoundnessTesterOptions {
   // TODO separate further into options for soundness testing and options for addFiles files.
   // TODO somehow include the information from KnownUnsoundness here, to allow for configurable known unsoundnesses (will violate package dependencies if done naively).
    ...
    /**
     * Can addFiles files be generated if needed.
     */
    private boolean generate = false;

    /**
     * Force regeneration of addFiles files.
     */
     ...
     * If this path is wrong, then addFiles file generation will happen on too few files.
     */
    private Path rootDirFromMainDirectory = null;

    /**
     * The location of the addFiles file. If not present, the addFiles file is expected to be located in /test-resources/jalangilogfiles/.
     */
    private Optional<Path> explicitSoundnessLogFile = Optional.empty();

    /**
     * Use headless browser for browser for generating addFiles files in a browser.
     */
    private boolean nonInteractive = false;

    /**
     * The environment to generate the addFiles file in. If not present, an environment is heuristically selected.
     */
    private Optional<Logger.Environment> generatorEnvironmentExplicitly = Optional.empty();

    /**
     * The time (in seconds) the application is allowed to run when generating addFiles files. If not present, a default value is chosen (10 seconds at the time of writing).
     */
    private Optional<Integer> timeLimitExplicitly = Optional.empty();

    /**
     * The time (in seconds) the instrumentation is allowed to run when generating addFiles files. If not present, a default value is chosen (10 seconds at the time of writing).
     */
    ...
    /**
     * Use addFiles files regardless of their sha-value. For development only.
     */
    private boolean ignoreShaDifference = false;

    /**
     * Do not compress addFiles files.
     */
    ...
    /**
     * Update the sha in addFiles files without regenerating the entire addFiles file. For development only.
     */
    private boolean forceUpdateSha = false;

    /**
     * Generates the addFiles files before performing the analysis
     */
