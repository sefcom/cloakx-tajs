...
public class JSGlobal {
     ...
     */
    public static Value evaluate(DynamicDOMObject dynObj, CallInfo call, Solver.SolverInterface c) {
        State state = c.getState();
        if (dynObj.equalsString(ChromeAPI.TABS_EXECUTE_SCRIPT )) {
    ...
    public static Value evaluate(DOMObjects domObject, CallInfo call, Solver.SolverInterface c) {
        State state = c.getState();

        switch (domObject) {
             case ELEMENT_SET_ATTRIBUTE :
                 // first paramater is attribute name, second is attribute value
                 Value arg = FunctionCalls.readParameter(call, state, 1);
                 Value evalString = arg.restrictToStr();
                 Value evaled = evalString.isNone() ? Value.makeNone() : evaluateEvalStringArgument(evalString, call, state, c);
                 return evaled.join(arg.restrictToNotStr());
        }
    ...
    private static Value evaluateEvalStringArgument(Value evalString, CallInfo call, State state, GenericSolver<State, Context, CallEdge, IAnalysisMonitoring, Analysis>.SolverInterface c) {
        ...
        if (call.getNumberOfArgs() > 0){
            SourceLocation callSL = callNode.getSourceLocation();
            PropVarOperations propvarops = c.getAnalysis().getPropVarOperations();
            propvarops.declareAndWriteVariable(callSL.toString(), call.getArg(0), false);
        }
