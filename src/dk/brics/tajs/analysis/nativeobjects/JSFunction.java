...
public class JSFunction {
    ...
    public static Value evaluate(ECMAScriptObjects nativeobject, final CallInfo call, final Solver.SolverInterface c) {
        ...
        switch (nativeobject) {
            ...
            case FUNCTION_BIND: { // 15.3.4.5
                // FIXME: this is doing what's required, but not the proper way to do this.  Plus it might also break if more specific DOM operation tracking is added.
                //call.getFunctionValue()
                //Function fun = ((CallNode)call.getSourceNode()).();
                //ObjectLabel fn = instantiateFunction(fun, state.getScopeChain(), n, state, c);
                String erikwazhere="";
                CallNode cn = ((CallNode)call.getSourceNode());
                Value fn_to_bind = state.getRegisters().get(cn.getBaseRegister());
                for (int i=0; i < cn.getNumberOfArgs(); i++){
                    if (!fn_to_bind.addBindOverride(state.getRegisters().get(cn.getArgRegister(i)))){
                        // if the override cannot be done, this might be a prototype without a
                        // defined function, in this case, there should be a register
                        // that isn't listed as an argument but that can be used this
                        // will be the instantiation of the function to bind to.
                        for (int j=0; j < state.getRegisters().size(); j++){
                            boolean found = false;
                            for (int arg_reg=0; arg_reg < cn.getNumberOfArgs(); arg_reg++){
                                if (cn.getArgRegister(arg_reg) == j){
                                    found = true;
                                }
                            }
                            if (!found ){
                                Value temp = state.getRegisters().get(j);
                                if (temp != null){

                                    if (temp != null &&  temp.getObjectLabels() != null &&
                                            temp.addBindOverride(state.getRegisters().get(cn.getArgRegister(i)))){
                                        fn_to_bind = temp;
                                    }
                                }
