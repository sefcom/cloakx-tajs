...
public class Conversion {
    ...
    private static Value defaultValue(ObjectLabel obj, Hint hint, Solver.SolverInterface c) {
        ...
        if (hint == Hint.STR) {
            ...
            Value tostring = pv.readPropertyValue(Collections.singleton(obj), "toString").restrictToStr();
            ...
            //erikt trying to fix endless loop
            if (tostring.getObjectLabels().contains(obj)){
                // do nothing keepprior result
                //result = tostring.restrictToNotObject();
                //tostring = tostring.restrictToStr();
            ...
            if (isMaybeNonCallable(tostring) || result.isMaybeObject()) {
                ...
                if (tostring.getObjectLabels().contains(obj)){
                    // do nothing keepprior result
                    //result = valueof.restrictToNotObject();
    ...
    public static Value toPrimitive(Value v, Hint hint, Solver.SolverInterface c) {
        ...
        for (Iterator<ObjectLabel> it = v.getObjectLabels().iterator(); it.hasNext(); ) {
            ...
            if (v.getObjectLabels().size() == 64 && ol.toString().contains("@Array#node755")){
