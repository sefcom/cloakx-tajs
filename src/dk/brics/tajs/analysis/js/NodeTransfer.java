...
public class NodeTransfer implements NodeVisitor {
    ...
    public void visit(ConstantNode n) {
        ...
        switch (n.getType()) {
            ...
            case NUMBER:
                ...
                break;
            case STRING:
                v = Value.makeStr(n.getString(), n.getSourceLocation());
                break;
            default:
    ...
    public void visit(WritePropertyNode n) {
        // get the base value, coerce with ToObject
        if (n.toString().contains("id") || (n.getPropertyString() !=null && n.getPropertyString().equals("A"))){
            String erikwazhere="?";
        }
        Value baseval = c.getState().readRegister(n.getBaseRegister());
        baseval = UnknownValueResolver.getRealValue(baseval, c.getState());
    ...
    public void visit(CallNode n) {
        ...
        if (n.getFunctionRegister() != AbstractNode.NO_VALUE) // old style call (where the function is given as a variable read)
            FunctionCalls.callFunction(new OrdinaryCallInfo(n, c.getState()) {
                ...
                public Value getThis() {
                    if (n.getBaseRegister() == AbstractNode.NO_VALUE) {
                        ...
                        // we need to error on the side of defined not undef -erikt
            ...
            if (n.toString().contains("v40") || n.toString().contains("join',v41,v37")){
                String erikwazhere="";
            }
            Value baseval = c.getState().readRegister(n.getBaseRegister());
            baseval = UnknownValueResolver.getRealValue(baseval, c.getState());
            ...
            // If callNode is for setAttribute then mimic the set by creating a WritePropertyNode with the registers. -eet
           /*if (n.getPropertyString() != null && n.getPropertyString().equals("setAttribute")) {
                //WritePropertyNode(int base_reg, String property_str (property_reg), int value_reg, Kind kind, boolean decl, SourceLocation location) {
                if (n.getNumberOfArgs() > 1) {
                    WritePropertyNode wpn = new WritePropertyNode(n.getBaseRegister(), n.getArgRegister(0), n.getArgRegister(1), false, n.getSourceLocation());
                    this.visit(wpn);
                }
            }*/
