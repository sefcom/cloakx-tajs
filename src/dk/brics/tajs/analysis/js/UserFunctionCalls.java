...
//        }
        ...
        pv.writePropertyWithAttributes(prototype, "bind", Value.makeObject(InitialStateBuilder.FUNCTION_PROTOTYPE).setAttributes(true, true, false));
    ...
    public static void enterUserFunction(ObjectLabel obj_f, CallInfo call, boolean implicit, Solver.SolverInterface c) {
        ...
        if (obj_f_sc == null || prototype.isNone()) {
            /*throw new AnalysisException("error couldn't find scope or prototype for function\n"
                    + " scope retrieved: " + obj_f_sc + "\nprototype: "+ prototype +"\nlabel: " + obj_f +
                    "\nnode: " + call.getSourceNode() + "\nexecutionCTX: " + call.getExecutionContext());*/
        ...
        c.withState(edge_state, () -> {
                    ...
                    if (call.isConstructorCall()) {
                        ...
                        if (f.bindOverrideIsSet()) {
                            rawThisVal = f.getBindOverride(0);
                        }
                        if (f.isStrict()) {
                            thisVal = rawThisVal;
                        } else {
                    ...
                    for (int i = 0; i < f.getParameterNames().size() || i < (call.isUnknownNumberOfArgs() ? numberOfUnknownArgumentsToKeepDisjoint : call.getNumberOfArgs()); i++) {
                        Value v;
                        if (i == 0 && f.bindOverrideIsSet() && (i+1) < f.nbrBindOverrides()){
                            v = f.getBindOverride(i+1);
                            //v = call.getArg(i);
                        } else {
                            v = call.getArg(i);
                        }
                        ...
                        //FIXME: this will only handle 1 param??? -erikt
                        varobj.setValue(v);
                    }
    ...
    private static void returnToCaller(AbstractNode node, Context caller_context, Context edge_context, boolean implicit, Value returnval, boolean exceptional, Function f, State state, Solver.SolverInterface c) {
        ...
        if (exceptional) {
            ...
            // ErikT I think this is the problem, why are we skipping if return is none?
            // This was inside above if previously, but would cause the program to stop moving forward because
