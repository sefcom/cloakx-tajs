...
public class PropVarOperations {
    ...
    public Value readPropertyDirect(ObjectLabel objlabel, Str propertystr) {
        State state = c.getState();
        if (propertystr.equals("anonfun3")){
            String erikwazhere="";
        }
        ...
        if (propertystr.isMaybeSingleStr()) {
            Value v = null;
            if (propertystr.getStr().equals("id")) {
                String name = DOMRegistry.MayMaps.ELEMENTS_BY_ID.name();
                v = state.getExtras().getFromMayMapValues(name);
            }
            // if property is bind, we don't want prototype but pointer to concrete function
            /*if (propertystr.getStr().equals("bind")) {
                v = Value.makeObject(objlabel);
            }*/
            if (v != null) {
                return v;
            } else {
    ...
    private void writeProperty(ObjectProperty objprop, Value value,
        ...
        if (c.getNode().toString().contains("call[v40,'join',v41")){
            String erikwazhere="";
        }
        State state = c.getState();
        ...
        if (writeable.isMaybeTrue()) {
            if (force_weak || writeable.isMaybeFalse() || !objprop.getObjectLabel().isSingleton() || objprop.getProperty().isFuzzy() || (!allow_overwrite && oldvalue.isMaybePresent())) { // weak update
                ...
                //value = UnknownValueResolver.join(oldvalue, value, state);
            ...
            pt.transferIsWriteOperation(); //erikt
            pt.complete();
        }
    }
    ...
    public Set<ObjectLabel> writeVariable(String varname, Value value, boolean set_modified, boolean not_invoke_setters) {
        ...
        for (Iterator<Set<ObjectLabel>> it = ScopeChain.iterable(state.getExecutionContext().getScopeChain()).iterator(); it.hasNext(); ) {
            ...
            // as far as I can tell this is an efficiency improvement, but for some reason it's exiting early when
            // iterating down 3 or more contexts, so disabled for now.
            if (definitely_found)
                break;
        }
        pf.transferIsWriteOperation(); //erikt trying to fix improper propagation of undef
        pf.complete();
        if (log.isDebugEnabled())
