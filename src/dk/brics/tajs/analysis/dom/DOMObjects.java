...
public enum DOMObjects implements HostObject {
    ...
    CHROME_RUNTIME_SEND_MESSAGE("Chrome.runtime.sendMessage"),
    CHROME_RUNTIME_ONMESSAGE("Chrome.runtime.onMessage"),
    CHROME_RUNTIME_GETURL("Chrome.runtime.getURL"),
    CHROME_RUNTIME_ONMESSAGE_ADDLISTENER("Chrome.runtime.onMessage.addListener"),
    CHROME_DUMMY("Chrome.dummy"),
    ...
    ELEMENT_WK_CREATE_SHADOWROOT("Element.webkitCreateShadowRoot"),
    ELEMENT_CREATE_SHADOWROOT("Element.createShadowRoot"),
    ELEMENT_INNER_HTML("Element.innerHTML"),
    ...
    HTMLELEMENT_CLICK("HTMLElement.prototype.click"),
    ...
    public static void evaluateDOMSetter(ObjectLabel objlabel, Str prop, Value rhsValue, Solver.SolverInterface c) {
        ...
        if (prop.isMaybeSingleStr() && (prop.getStr().equals("innerHTML") || prop.getStr().equals("outerHTML"))) { // FIXME: what about fuzzy property names for innerHTML/outerHTML? emit unsoundness warning? (GitHub #400)
            pt.add(() -> {
                ...
                String source = innerHTMLText.getStr()==null ? "" : innerHTMLText.getStr();
                HTMLParser parser = new HTMLParser(source , loaderLocation.getLocation(), new DynamicLocationMaker(loaderLocation));
                DOMBuilder.registerHTML(parser.getHTML(), c, innerHTMLText);
