...
public class DOMBuilder {
    ...
    public static void registerHTML(Source document, Solver.SolverInterface c) {
        registerHTML(document, c, null);
    }

        /**
         * Build model of the HTML page, as it looks when loaded, before scripts are invoked.
         */
    public static void registerHTML(Source document, Solver.SolverInterface c, Value value) { // TODO: (#118) more precise models of the HTML DOM?
        State s = c.getState();
        // Ignore HTML content?
        ...
        for (Element element : document.getAllElements()) {
            ...
            ObjectLabel label = DOMFunctions.getHTMLObjectLabel(element.getName(), c);
            if (label != null) {
                if (value != null){
                ...
                if (id != null) {
                    ...
                    s.getExtras().addValueToMayMap(DOMRegistry.MayMaps.ELEMENTS_BY_ID.name(), id, value);
