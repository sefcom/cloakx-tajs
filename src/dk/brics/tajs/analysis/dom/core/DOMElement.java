...
public class DOMElement {
    ...
    public static void build(Solver.SolverInterface c) {
        ...
        createDOMFunction(PROTOTYPE, DOMObjects.ELEMENT_WK_CREATE_SHADOWROOT, "webkitCreateShadowRoot", 1, c);
        createDOMFunction(PROTOTYPE, DOMObjects.ELEMENT_CREATE_SHADOWROOT, "createShadowRoot", 1, c);
    ...
    public static Value setAttributeModel(Set<ObjectLabel> receivers, Value attributeName, Value attributeValue, Solver.SolverInterface c) {
        PropVarOperations pv = c.getAnalysis().getPropVarOperations();
        if (attributeName.isMaybeSingleStr() && (attributeName.getStr().equals("name") || attributeName.getStr().equals("id"))) {
            pv.writeProperty(singleton(HTMLCollection.INSTANCES), attributeValue, Value.makeObject(receivers));
        }

        //Make specific property writes to HTML object
        pv.writeProperty(receivers, attributeName, attributeValue);
    ...
    public static Value evaluate(DOMObjects nativeObject, CallInfo call, Solver.SolverInterface c) {
        ...
        switch (nativeObject) {
            ...
            case ELEMENT_SET_ATTRIBUTE: {
                ...
                Value attributeValue = JSGlobal.evaluate(nativeObject, call, c);
                if (attributeValue.isNone()){
                    attributeValue = Conversion.toString(FunctionCalls.readParameter(call, s, 1), c);
                }
            ...
            case ELEMENT_WK_CREATE_SHADOWROOT:
            case ELEMENT_CREATE_SHADOWROOT: { // rudimentry support for createShadowRoot
                DOMFunctions.expectParameters(nativeObject, call, c, 0, 0);
                // FIXME: NOT really sure what to default this to -Erik
                String t = "DIV";
                return Value.makeObject(DOMFunctions.getHTMLObjectLabel(t, c));
                // may throw on bad syntax
