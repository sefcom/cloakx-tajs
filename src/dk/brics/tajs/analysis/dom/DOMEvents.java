...
public class DOMEvents {
    ...
    private static final Value sendMessageEvent;
    ...
    static {
        ...
        sendMessageEvent = Value.makeNone();
    ...
    private static DOMRegistry.MaySets mapEventTypeToMaySetKey(EventType kind) {
        switch (kind) {
            ...
            case SEND_MESSAGE:
                return DOMRegistry.MaySets.SEND_MESSAGE_EVENT_HANDLERS;
    ...
    private static void triggerEventHandler(AbstractNode currentNode, State currentState, EventType type,
                                            boolean requiresStateCloning, Solver.SolverInterface c,
                                            List<Value> eventArgs, Set<ObjectLabel> handlers) {
        ...
        if (handlers.isEmpty()) {
            /*if (currentNode instanceof CallNode) {
                FunctionCalls.callFunction(new FunctionCalls.OrdinaryCallInfo((CallNode) currentNode, currentState), c);
            }*/
            return;
        }
        ...
        c.withState(callState, () -> {
            for (ObjectLabel l : handlers) {
                //addFiles.info("Triggering eventHandlers <" + type + ">: " + l);
                //String erikwazhere="";
            }
            ...
            List<Value> args;
            if (eventArgs == null){
                args = event.isNone() ? newList() : Collections.singletonList(event);
            } else {
                args = eventArgs;
            }
    ...
    private static void triggerEventHandler(EventType type, Solver.SolverInterface c){
        triggerEventHandler(type, c, null);
    }

    protected static void triggerEventHandler(EventType type, Solver.SolverInterface c, List<Value> eventArgs) {
        ...
        Set<ObjectLabel> handlers = c.getState().getExtras().getFromMaySet(maysetKey.name());
        if (handlers.isEmpty() && type == EventType.SEND_MESSAGE){
            handlers = new HashSet<>();
            ObjectLabel obj = ObjectLabel.make( c.getNode(), ObjectLabel.Kind.OBJECT);
            handlers.add(obj);
        }
        triggerEventHandler(c.getNode(), c.getState(), type, requiresStateCloning, c, eventArgs, handlers);
    }
    ...
    public static void emit(EventDispatcherNode n, Solver.SolverInterface c) {
        ...
        if (n.getType() == EventDispatcherNode.Type.DOM_OTHER) {
            ...
            triggerEventHandler(EventType.SEND_MESSAGE, c);
        }
