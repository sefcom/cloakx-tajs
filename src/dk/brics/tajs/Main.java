...
import dk.brics.tajs.monitoring.ConstantValueMonitor;
...
public class Main {
    ...
    private static URL htmlFile = null;
    ...
    public static void main(String[] args) {
        try {
            initLogging();
            //SolverSynchronizer ss = new SolverSynchronizer();
            ...
            TotalsLog.report("Error: " + e.getMessage());
    ...
    public static Analysis init(OptionValues options, IAnalysisMonitoring monitoring, SolverSynchronizer sync) throws AnalysisException {
        ...
        try {
            ...
            log.error("Error unable to load and parse " + use.getMessage());
            return null;
        }
        ...
        analysis.getSolver().init(fg, document, htmlFile);
    ...
    private static IAnalysisMonitoring addOptionalMonitors(IAnalysisMonitoring monitoring) {
        ...
        // Add Constant Value Tracking
        if (Options.get().isSearchEnabled()) {
            ConstantValueMonitor cvm = new ConstantValueMonitor(Options.get().getSearchFor(), Options.get().getSearchType());
            extraMonitors.add(cvm);
        }
