...
abstract public class SolverSynchronizer {
    ...
    synchronized void waitIfSingleStep() {
        if (single_step && shouldWait())
            try {
                while (shouldWait()) {
                    waiting();
                    wait(50); // TODO: wrap in while loop to check condition (to avoid spurious notifications)
                }
    ...
    abstract public boolean shouldWait();
    /**
    ...
    abstract public void unMarkActiveBlock();
    /**
