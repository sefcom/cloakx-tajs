...
import dk.brics.tajs.util.Colorize;
...
public class FlowGraph {
    ...
    /**
     * Produces a String representation of each function in this flow graph.
     *
     */
    private static final String BOLD_RED = "\033[1;31m";

    private static final String FAIL_COLOR = "\033[1;31m";
    private static final String BOLD_GREEN = "\033[1;32m";
    private static final String NONE = "\033[0m\033[49m";

    public String toStringFG(boolean mainOnly) {
        StringBuilder output = new StringBuilder();
        List<Function> sortedFunctions = newList(functions);
        sortedFunctions.sort(Comparator.comparingInt(Function::getOrder));
        int analyzedBlocks = 0;
        int totalBlocks = 0;
        int analyzedFuncs = 0;
        int partialAnalyzedFuncs =0;
        int totalFuncs = 0;
        Set<String> sources = new HashSet<>();

        for (Function function : sortedFunctions) {
            //sources.add(function.getSourceLocation())

            if (!function.isMain() && mainOnly){
                continue;
            }
            ...
            String analysisStatus = function.getAnalysisInfo(this);
            StringBuilder fnInfo = new StringBuilder();
            fnInfo.append(function.getIndex() + " " + fnName + " in '" + fpath.getFileName() + "' " + function.getSourceLocation().toShortString(true));
            fnInfo.append(" " ).append(analysisStatus);
            fnInfo.append("\n");
            // only going to print function statement if fully analyzed
            if (function.isFullyAnalyzed(this)) {
                analyzedFuncs++;
                output.append("\n");
                output.append(Colorize.green(fnInfo.toString())).append("\n");

            } else {
                if (function.getAnalysisCount(this)>0){
                    partialAnalyzedFuncs++;
                }
                output.append(fnInfo);
                output.append(function.toStringFG(this));
            }
            totalFuncs++;
            analyzedBlocks += function.getAnalysisCount(this);
            // getAdjustedNumberOfBlocks should be after getAnalysisCount.
            totalBlocks += function.getAdjustedNumberOfBlocks();

        }
        StringBuilder outTots = new StringBuilder();
        TotalsLog.setAnalysisTotals(analyzedFuncs, partialAnalyzedFuncs, totalFuncs, analyzedBlocks, totalBlocks);
        TotalsLog.writeOutTotals();
        output.append(outTots);
