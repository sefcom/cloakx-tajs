...
public class BasicBlock implements Serializable {
    ...
    private boolean errorCaughtDuringProcessing = false;

    public void increaseTimesAccessed(){
        timesAccessed++;
    }
    public boolean hasBeenAccessed(){
        return timesAccessed > 0;
    }
    public int getTimesAccessed(){
        return timesAccessed;
    }
    public void errorCaught(){
        errorCaughtDuringProcessing = true;
    }
    public boolean errorWasCaught() {
        return errorCaughtDuringProcessing;
    }
    ...
    /**
     * Returns a shorter string description of this block.
     */
    public String toShortString(int sizeOfWorklist) {
        StringBuilder s = new StringBuilder();
        StringBuilder intro = new StringBuilder();
        String finalLineNumber = "";
        for (AbstractNode n : getNodes()) {

            if (intro.length() == 0 && n.getSourceLocation().toShortString().length() > 0){
                Path fpath = FileSystems.getDefault().getPath(n.getSourceLocation().toUserFriendlyString(false));
                if (!Files.isReadable(fpath)){
                    continue;
                }
                if (sizeOfWorklist > 1) {
                    intro.append("size=" + sizeOfWorklist + "\n");
                }
                String fnName = this.getFunction().getName();
                if (this.getFunction().isMain()) {
                    fnName = "main";
                }
                if (fnName != null) {
                    intro.append("\tBlock #" + index + " " + fnName + " " + fpath.getFileName() + n.getSourceLocation().toShortString() + "");
                } else {
                    continue;
                }
            }
            s.append(n);
            s.append(" ");
            s.append(n.getSourceLocation().toShortString());
            s.append(" ");
            finalLineNumber = n.getSourceLocation().toShortString();

        }
        if (s.length() > 0 && intro.length() > 0) {
            intro.append("->");
            intro.append(finalLineNumber);
            intro.append(" ");
            s.insert(0, intro);
            return s.toString();
        }
    ...
    public boolean neverHasState() {

        if (getNodes().size() == 1) {
            AbstractNode node = getNodes().get(0);
            return node instanceof ExceptionalReturnNode || node instanceof EndForInNode;
        }
        return false;

    }

    public boolean unalayzedTAJSBlock(FlowGraph flowGraph) {
        for (AbstractNode node : getNodes()) {
            // if not User Code but it's been accessed then this is NOT an unanalyzedTAJS Block
            if (flowGraph.isUserCode(node) ){
                return false;
            }
            if (node.hasBeenAccessed() ){
                return false;
            }
     ...
     *
     */
    public String toStringFG(FlowGraph flowgraph, boolean nowhitespace) {
        StringBuilder output = new StringBuilder();
        for (AbstractNode node : getNodes()) {

            AbstractNode d = node.getDuplicateOf();
            String tabs = "\t\t\t";
            if (nowhitespace){
                tabs = "";
            }
            ...
            if (flowgraph.isUserCode(node)) {
                output.append(tabs).append("node#").append(node.getIndex());

                if (d != null) {
                    output.append("(~" + d.getIndex() + ")");
                }
                output.append(" ");
                output.append(Strings.escape(node.toString()));
                output.append(" ");
                output.append(node.getSourceLocation().getLineNumber());
                output.append(":");
                output.append(node.getSourceLocation().getColumnNumber());
                output.append(" ");

            } else {

                output.append(tabs).append("\t\t\t\t\t\tnode#").append(node.getIndex());
                output.append("TAJS Code " );
                output.append(Strings.escape(node.toString()));
    ...
    public void setNodes(List<AbstractNode> newNodes) {
        this.nodes = newNodes;
    }
}
